...
**ダッシュボード。** 実績は塗りつぶし、予測はハッチ、95%予測区間は帯で描いています。
その予測を出したモデル名と、ナイーブ予測に対する成績（MASE）を真下に併記しています。

![QuoteCast ダッシュボード（ライト）](docs/images/dashboard-light.png)

**同じ画面のダークテーマ。** ライトとダークはどちらかのついでではなく、同格に設計しています。

![QuoteCast ダッシュボード（ダーク）](docs/images/dashboard-dark.png)

**見積もり一覧。** 一日中触る人のための密度を優先しました。
ステータスはラベル付きバッジ、金額は等幅数字、有効期限は残り日数がそのままバーの長さになります。
...
![見積もり一覧（英語表示）](docs/images/quotations-en.png)
...
| **需要予測** | 今後の見積もり件数・金額を予測し、95%予測区間を帯としてグラフに描画 |
| **営業分析** | 受注率・進行中金額・ステータス内訳・顧客別金額・期限が近い見積もり |
| **絞り込み** | サーバー側検索とステータス・日付フィルター。残り日数はバーの長さで表示 |
| **多言語・テーマ** | 日本語を既定とし英語切り替えに対応。ライト／ダークを同格に設計 |
...
**6. 根拠が画面に出る。** どのモデルが選ばれ、ナイーブ予測に対してどれだけの成績だったか
（例: `Theta法 · MASE 0.92`）をダッシュボードに表示し、95%予測区間は棒の背後に帯として描きます。
実績は塗りつぶし、予測はハッチと輪郭で描き分けるため、グレースケールでも両者が混ざりません。
...
判断をコードの中に埋もれさせないよう、2つのドキュメントに記録しています。

- **[PRODUCT.md](./PRODUCT.md)** — 誰のための製品か、何を可能にするか、今後の変更が守るべき制約。
- **[DESIGN.md](./DESIGN.md)** — トークン体系、タイポグラフィとレイアウトの規則、
  コンポーネントの状態、モーションの予算、多言語対応の制約、アクセシビリティの下限。

要点として、この画面は「作業のための面（Operate）」なので、表現が作業を邪魔しないことを最優先にしています。
色は状態にのみ使い、色だけで意味を運ばせない。比較しうる数字はすべて等幅数字で組む。
ライトとダークはどちらかのついでではなく同格に設計する。
